...
@section('content')
    <div class="mb-6 text-center">
        <span class="mb-3 inline-flex h-14 w-14 items-center justify-center rounded-xl bg-automotive-500/30 text-3xl">{{ $config['icon'] }}</span>
    ...
    @if($config['register'])
        <p class="mt-2 text-center text-sm text-automotive-400">
            Não tem conta? <a href="{{ route('register') }}" class="text-wrench-400 hover:underline">Criar conta gratuita</a>
        </p>
    @endif
@endsection
