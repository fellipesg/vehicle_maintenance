...
Resumo de procedência (capa PDF e strip web): contador compacto (`N com selo · M declarada(s)`) + linha de pontos `.prov-dot` (10px, gap 4px) — disco teal cheio = selo, anel tracejado âmbar = declarada. Web: até 16 pontos visíveis, depois `+N`. PDF capa: um ponto por manutenção com data mm/aa, 12 por linha. Não usar mais a barra segmentada `.prov-strip*`.
